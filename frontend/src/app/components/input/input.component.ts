...
@Component({
  ...
  template: `
    <label [for]="id" class="text-md justify-left block px-3 py-0 font-medium"
           [ngClass]="formControl.invalid && formControl.dirty ? ('text-red-800') : ('text-gray-900')" [innerHTML]="label" >
    </label>
    <div class="relative">
...
const errorsTexts: { [ keys: string ]: (value: string) => string } = {
  "min"       : (value: string) => {return `Il campo non può essere inferiore a ${value}`},
  "max"       : (value: string) => {return `Il campo non può essere superiore a ${value}`},
  "maxlength" : (value: string) => {return `Il campo non deve superare i ${value} caratteri`},
  "minlength" : (value: string) => {return `Il campo non deve essere inferiore a ${value} caratteri`},
  "pattern"   : (value: string) => {return value.includes('([+]39)') ? `Il numero di telefono non è valido` : `Campo non valido`},
  "email"     : ()              => {return `L'email non è valida`},
  "required"  : ()              => {return `Il campo è obbligatorio`},
}
