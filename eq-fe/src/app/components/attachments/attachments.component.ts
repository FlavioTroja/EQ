...
@Component({
  ...
  standalone: true,
  imports: [ CommonModule, FileUploadComponent, HorizontalScrollDirective, MatIconModule, HorizontalScrollDirective ],
  template: `
    <div class="bg-foreground rounded-md p-2 flex gap-2 h-60 default-shadow">
      <div *ngIf="!viewOnly" class="flex flex-col justify-around bg-white rounded-lg text-center cursor-pointer aspect-square h-full hover:bg-gray-100">
        <app-file-upload [images]="currentAttachment" label="{{label}}" (onUpload)="onUpload.emit($event)" [onlyImages]="onlyImages" />
      </div>

      <div appHorizontalScroll class="flex gap-2 lg:overflow-x-auto">
